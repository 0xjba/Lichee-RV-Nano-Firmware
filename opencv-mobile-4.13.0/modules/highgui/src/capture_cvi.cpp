...
static int load_sns_obj_library()
{
    ...
    else if (device_model == 3 || device_model == 5)
    {
        // licheerv nano
...
public:
    ...
    int open(int width = 1920, int height = 1440, float fps = 30);
...
{
    if (!sys.ready) { fprintf(stderr, "cvi: sys not ready\n"); return false; }
    if (!vpu.ready) { fprintf(stderr, "cvi: vpu not ready\n"); return false; }
    if (!sns_obj.ready) { fprintf(stderr, "cvi: sns_obj not ready\n"); return false; }
    if (!ae.ready) { fprintf(stderr, "cvi: ae not ready\n"); return false; }
    if (!awb.ready) { fprintf(stderr, "cvi: awb not ready\n"); return false; }
    if (!isp.ready) { fprintf(stderr, "cvi: isp not ready\n"); return false; }
    if (!cvi_bin.ready) { fprintf(stderr, "cvi: cvi_bin not ready\n"); return false; }
